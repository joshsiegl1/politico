...
namespace MenuSystem
{
  ...
  scaleMatrix = Matrix.CreateScale((float)GraphicsDevice.Viewport.Width/ virtualWidth, (float)GraphicsDevice.Viewport.Width/ virtualWidth, 1f);
        ...
        /// <summary>
        /// The default button texture that is shared by all buttons. 
        /// </summary>
        ...
        /// <summary>
        /// The default button pressed texture that is shared by all buttons
        /// </summary>
        ...
        /// <summary>
        /// the default loading circle used for displaying load times
        /// </summary>
        public Texture2D LoadingReticle
        {
            get { return loadingReticle; } 
        }

        /// <summary>
        /// the default text bar texture
        /// </summary>
        public Texture2D TextBar
        {
            get { return textBar; } 
        }
        ...
        protected override void LoadContent()
        {
            ...
            font = content.Load<SpriteFont>("Menu/Fonts/LogInFont");
            //blankTexture = content.Load<Texture2D>("Graphics/blank");
            //buttonTexture = content.Load<Texture2D>("button");
            //buttonpressedTexture = content.Load<Texture2D>("button_pressed");
            //loadingReticle = content.Load<Texture2D>("Graphics/loadingCircle");
            //textBar = content.Load<Texture2D>("Graphics/LoginBar");
        ...
        /// <summary>
        /// override allows for a delay in transition
        /// </summary>
        public void AddScreen(GameScreen screen, PlayerIndex? controllingPlayer, float WaitTime)
        {
            screen.ControllingPlayer = controllingPlayer;
            screen.ScreenManager = this;
            screen.IsExiting = false;
            screen.WaitTime = WaitTime;

            for (int i = 0; i < screensToUpdate.Count; i++)
                screensToUpdate[i].WaitTime = WaitTime;

            for (int i = 0; i < screens.Count; i++)
                screens[i].WaitTime = WaitTime; 
